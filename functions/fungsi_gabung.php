...
$menu ='<div class="l"></div><div class="r"></div>
		...
		<ul class="art-menu" id="sidebar-onmobile"><div class="logo-section">
                  <a href="/">
                    <h2>'.$nmsekolah.'</h2>
                  </a>
                </div>';
...
while($row=mysql_fetch_array($query)) {
	...
	if ($ak==$row[id]) $menu .='<a class="active" href="'.$link.'" target="'.$row['target'].'" class="active" ><span class="t"><i class="material-icons">'.$icons.'</i> '.$row[judul].'</span></a>';
	else $menu .='<a href="'.$link.'" target="'.$row['target'].'" ><span class="t"><i class="material-icons" style="color: #616161;">'.$icons.'</i> '.$row[judul].'</span></a>';
...
include "koneksi.php";
	...
	if ($row=mysql_fetch_array($query)) {
		...
		$atas = "<img src='../images/banneratas/gbanner$row[id].$row[jenis]' title='$row[judul]' width='100%' height='150'>";}
	}
