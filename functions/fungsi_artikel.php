...
list($userid,$user,$pass,$kategori) = explode("\|",$memberlog);
  ...
  $artikel .= "<tr><td><ul class='row-list'>";
  while ($row = mysql_fetch_array($query_result_handle))
  {
  	$del="";
    $penulis = ($row[pengirim] == "") ? "Anonymous" : $row[pengirim];
   $artikel .= "<li>
                <div class='row-items'>
                <i class='material-icons' style='color: #616161;'>description</i>
                <a href='index.php?id=artikel&kode=$row[id]' class='ver10' title='$row[judul]'> 
                  <h2 nm>$row[judul]</h2></a>

                  <span>Dibaca $row[visits] kali - Ditulis oleh $penulis</span>
...
<input type='submit' name='submit' value='Kirim' class='art-button'>&nbsp;<input type='reset' class='art-button' name='reset' value='Ulang'></form>";
	...
	$artikel .="<table class='art-article art-alumni-table' width=100% >"
             ."<tr>"
             ."<td>"
             ."<div>"
             ."<b>$row[nama]</b> <i>[$row[email]]</i><br>"
             ."<div class='isi-komentar'>$row[komentar]</div>"
             ."<div align='right'>$row[tgl]</div>"
             ."</div>"
             ."</td>"
             ."</tr>"
             ."</table>";
